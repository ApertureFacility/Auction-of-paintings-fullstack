...
---
Вот как можно красиво оформить этот блок для README:

---

## Тестирование торгов

1. Перейдите в любой лот из главной страницы — у него уже есть **история ставок**.

2. Откройте [Swagger-документацию](http://localhost:8000/docs) → найдите эндпоинт:

   ```
   POST /lots/{lot_id}/start
   ```

3. Эндпоинт требует ID лота:

   * Откройте страницу нужного лота.
   * В URL возьмите его `id` (например: `/lots/36` → id = `36`).
   * Передайте это значение в эндпоинт `lots/{lot_id}/start`.

4. Перезагрузите страницу лота.

5. Теперь можно **делать ставки**, пока не истечёт таймер ⏳.
